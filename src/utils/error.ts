...
export function getApiErrorMessage(error: unknown, options?: { conflictMessage?: string; defaultServerMessage?: string }): string {
    const defaultServer = options?.defaultServerMessage ?? 'Wystąpił błąd po stronie serwera. Spróbuj ponownie później.'
